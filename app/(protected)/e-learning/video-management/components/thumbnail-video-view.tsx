...
interface ThumbnailFormType {
  isOpen: boolean;
  item: LearningVideos | null;
  closeForm: () => void;
  openForm: (thumb: LearningVideos | null) => void;
}
...
export function ThumbnailFormProvider({ children }: { children: ReactNode }) {
  ...
  const openForm = (thumb: LearningVideos | null) => {
    setItem(thumb);
    setIsOpen(true);
  };
  ...
  return (
    <>
      <div className={cn("rounded-sm", className)}>
        <video controls src={item?.video} className="w-full h-auto" />
      </div>
    </>
  );
}
