...
export function Check({ id }: { id: number }) {
  const [checked, setChecked] = useState(false);
  ...
  useEffect(() => {
    ...
    setCheckAll(pageIds.every((item) => selectedIds.includes(item)));
  ...
  return (
    <span className="text-primary-500">
      {selectedIds.length > 0 ? (
        ...
        <>{"Please select the record(s) that you wish to delete."}</>
      )}
...
export function BulkDeleteAction({
    ...
    setOption({
      ...
      description:
        selectedIds?.length > 1
          ? `Are you sure you want to delete the ${selectedIds?.length} messages? This action cannot be reversed.`
          : `Are you sure you want to delete the message? This action cannot be reversed.`,
    });
  }, [selectedIds]);
  ...
  const onModalConfirmHandler = async ({
    ...
    if (action === "all") {
      ...
      toast({
        ...toastSuccessMessage,
        description: `${count} Messages Deleted Successfully`,
      });
      applyFiltersHandler();
      router.refresh();
      return;
    ...
    toast({
      ...
      description:
        selectedIds?.length > 1
          ? `${selectedIds?.length} Messages Deleted Successfully`
  ...
  return (
    <>
      <Tooltip content="Delete records">
        <Button
          variant="outline"
          size="icon"
