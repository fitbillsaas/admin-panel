...
interface CouponFormType {
  ...
  item: User[] | null;
  closeForm: () => void;
  openForm: (coupon: User[] | null) => void;
}
...
export function CouponFormProvider({ children }: { children: ReactNode }) {
  ...
  const openForm = (coupon: User[] | null) => {
    setItem(coupon);
    setIsOpen(true);
  };
...
const formSchema = z.object({
  ...
  discount_usage: z
    .string()
    .nonempty({
      message: "Redemption limit per person is required",
    })
    .refine((value) => /^\d*\.?\d+$/.test(value), {
      message: "Invalid number format",
    })
...
export function FormTemplate({ className }: FormTemplateProps) {
  const { closeForm, item } = useCouponForm();
              ...
              <FormItem>
                <FormControl>
                  <Input
                    ...
                    maxLength={2}
                    {...field}
                    placeholder="Redemption Limit per person"
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
